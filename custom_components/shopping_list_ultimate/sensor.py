...
async def async_setup_entry(hass: HomeAssistant, entry: ConfigEntry, async_add_entities: AddEntitiesCallback) -> None:
    ...
    async_add_entities([LastScannedSensor(coordinator, entry), CountSensor(coordinator, entry, "known_products", "Shopping List Ultimate known products", "mdi:barcode"), CountSensor(coordinator, entry, "total_scans", "Shopping List Ultimate total scans", "mdi:barcode-scan")])
...
class LastScannedSensor(BaseSensor):
    def __init__(self, coordinator, entry):
        super().__init__(coordinator, entry, "last_scanned_product", "Shopping List Ultimate last scanned product", "mdi:barcode-scan")
    @property
    def native_value(self):
    ...
    def extra_state_attributes(self):
        ...
        if not product:
            return {"found": False}
        return {"barcode": product.get("barcode"), "name": display_name(product), "brand": product.get("brand"), "image": product.get("image"), "found": bool(product.get("name")), "source": product.get("source")}
